...
> - 调试功能依赖 cjdb 调试器，cjdb 依赖特定版本的 Python 环境，具体版本依赖说明请查阅 [仓颉官方文档](https://cangjie-lang.cn/docs)（路径：工具指南 -> 调试工具 -> Python 扩展）。 建议优先将符合要求的 Python 解释器配置到系统环境变量中，插件启动时将自动检测并加载。 若系统未检测到合适版本，或需要指定特定的解释器路径，在 VSCode 设置中导航至 Cangjie > Debug > Tools > Python Path 进行手动配置。
...
- 单击顶部调试工具栏上的图标控制程序，包括单步执行、步入、步出、继续、暂停、重启或停止程序。
  ...
  ![debugControl1](./figures/debugControl1.png)
...
![debugTimelineShow](./figures/debugTimelineShow.png)
...
![enableDAPCommunicationLog](./figures/enableDAPCommunicationLog.png)
