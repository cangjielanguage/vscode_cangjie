...
VSCode 打开仓颉工程中的 .cj 文件，即可看到代码未使用符号置灰高亮效果。VSCode 不同主题显示的代码置灰高亮颜色不同，支持对代码变量、类、结构体、函数、函数参数等进行未使用置灰高亮显示和快速修复。

![signaturehelp](./figures/unusedSymbol.png)

鼠标悬停在未使用符号名称上，点击快速修复，可以快速删除对应的符号。

![signaturehelp](./figures/unusedSymbolQuickFix.png)


## 快速修复

VSCode 打开仓颉工程中的 .cj 文件，当代码存在支持快速修复的诊断报错时，鼠标悬停在报错的代码上，点击快速修复。

![import_symbol_hint](./figures/quickFixImportSymbolHint.png)

选择快速修复项，即可应用快速修复。

![do_import_symbol](./figures/quickFixDoImportSymbol.png)

目前支持自动导入未定义符号、删除未使用的 import 语句和删除未使用符号三种诊断报错的快速修复。