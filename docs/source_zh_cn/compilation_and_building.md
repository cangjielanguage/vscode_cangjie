...
> **注意：**
>
> `customized-option` 的条件不能设置内置的条件（@When[os == "Linux"] 不能作为 `customized-option` 的条件，即 "cfg1" : "--cfg =\"os=Linux\"" 是不允许的），只能添加开发者自定义条件。具体请参见 《仓颉编程语言开发指南》 的[条件编译](https://cangjie-lang.cn/docs?url=%2F1.0.3%2Fuser_manual%2Fsource_zh_cn%2Fcompile_and_build%2Fconditional_compilation.html) 章节。
...
![extraLibAdd](./figures/extraLibAdd.png)
