...
1. 在 VSCode 中搜索 Remote - SSH，单击安装按钮。

    ![remotessh1](./figures/remotessh1.png)
...
仓颉 SDK 主要提供了仓颉语言编译命令（cjc）、仓颉语言官方包管理工具（Cangjie Package Manager，简称 CJPM），以及仓颉格式化工具（Cangjie Formatter，简称 cjfmt）等命令行工具。正确安装并配置仓颉 SDK 后，可使用工程管理、编译构建、格式化、静态检查和覆盖率统计等功能。开发者可以离线手动安装 SDK。在官网下载 SDK 安装包，并在本地安装部署仓颉 SDK。

### 下载 SDK

开发者可以自行前往[仓颉下载中心](https://cangjie-lang.cn/download)，手动下载对应操作系统的仓颉 SDK。
...
macOS_x86_64 平台的 SDK 下载内容为：`cangjie-sdk-mac-x64-x.y.z.tar.gz`。
...
> **注意：**
>
> 目前 CJVM 后端只支持 Linux 系统，且仅在 0.51.4 及其过往版本中提供。一个工程目录下的多个模块只能使用同一种后端提供语言服务。
...
1. 找到 Cangjie Sdk: Option 选项，选择后端类型为 CJVM。

2. 找到 Cangjie Sdk Path: CJVM Backend 选项，输入 CJVM 后端 SDK 文件所在绝对路径。

3. 重启 VSCode 生效。

![CJVMSdkPathSet](./figures/cjvmSdkPathSet.png)
