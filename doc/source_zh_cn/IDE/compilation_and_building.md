...
若工程根目录的 `cjpm.toml` 文件使用 `workspace` 模式配置，则文件编辑区不提供运行按钮。
...
> **注意：**
>
> `customized-option` 的条件不能设置内置的条件（@When[os == "Linux"] 不能作为 `customized-option` 的条件，即 "cfg1" : "--cfg =\"os=Linux\"" 是不允许的），只能添加开发者自定义条件。具体请参见 《仓颉编程语言开发指南》 的[条件编译](../../../user_manual/source_zh_cn/compile_and_build/conditional_compilation.md) 章节。
